...
# Adicionamos o usuario_id aqui
def exibir(lista_categorias_ativas, usuario_id):
    ...
    with col1:
        ...
        with st.form("form_nova_categoria", clear_on_submit=True):
            ...
            if botao_salvar:
                if nova_cat.strip():
                    # Passamos o usuario_id para o banco
                    sucesso = adicionar_categoria(nova_cat, usuario_id)
        ...
        if lista_categorias_ativas:
            for cat in lista_categorias_ativas:
                ...
                if c2.button("Excluir", key=f"del_cat_{cat}", type="secondary"):
                    # Passamos o usuario_id também na exclusão
                    remover_categoria(cat, usuario_id)
